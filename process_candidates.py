...
def fetch_latest_jd_from_slack(
    ...
    while True:
        ...
        for message in messages:
            ...
            if allow_test_header and re.match(r"(?i)^#\s*jd(?:\s*-\s*|\s+)(?:quick|test)\b", header):
                jd_text = "\\n".join(lines[1:]).strip()
                if jd_text:
                    return jd_text, message
    ...
    raise RuntimeError(
        ...
        + (" or '# JD-Quick' (legacy '# JD-Test' also accepted)." if allow_test_header else ".")
    )
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument("--model", default="gpt-5-nano", help="Model for JD widening and query generation")
    parser.add_argument("--scorer-model", default="gpt-5.4-mini", help="Model for candidate scoring (supports structured output)")
